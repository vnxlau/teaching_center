...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get messages where current parent is sender or recipient
    ...
    console.error('Parent Messages API error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Handle different recipient types (only staff for parents)
    ...
    console.error('Parent Messages API POST error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
