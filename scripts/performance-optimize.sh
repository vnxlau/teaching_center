...
if [ ! -f "database/add-indexes.sql" ]; then
    print_error "Database indexes file not found at database/add-indexes.sql"
    exit 1
fi
...
if [ -f ".env.production" ]; then
    ...
    npx prisma db execute --file ./database/add-indexes.sql
